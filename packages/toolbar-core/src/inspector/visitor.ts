...
export function createTransformVisitor(options: {
	relativePath: string;
}): TraverseOptions {
		...
		JSXOpeningElement: {
			enter(path, state) {
				...
				const finalRelativePath =
					relativePath ||
					(rootPath && filename ? relative(rootPath, filename) : filename);
...
export function doJSXOpeningElement(
	...
	if (name && imports[name] && imports[name].source === "react-native") {
		const inspectorAttribute = jsxAttribute(
			...
			jsxExpressionContainer(
				objectExpression([
					...
					objectProperty(
						stringLiteral(INSPECTOR_NAME_ATTRIBUTE),
						stringLiteral(name),
					),
				]),
			),
		...
		const nameAttribute =
			name !== undefined
				? jsxAttribute(
						jsxIdentifier(INSPECTOR_NAME_ATTRIBUTE_DATA_KEY),
						stringLiteral(name),
					)
				: undefined;
